...
/*   Updated: 2024/08/13 16:41:35 by pstrohal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// initialize.c
void					setup_shell(t_shell *shell, char **envp, int argc,
							char **argv);
...
char					**append_array(char **arr, t_token *start,
							t_token *end);
void					append_cmd_node(t_cmd **head);
void					store_in_cmd(t_cmd **head, char **arr, char *vars,
							int *int_vars);

// parse_redir.c
int						check_redir(t_cmd **command, t_token **tkn_temp,
							int *err);
void					append_rdct_node(t_cmd **head, int type, char *filename,
...
int						*add_to_arr(char *vars, int *arr, char *added,
							int *int_vars);
...
int						is_closed(char *str, int q_count, int start,
							t_shell *shell);
